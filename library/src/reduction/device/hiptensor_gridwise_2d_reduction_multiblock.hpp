...
namespace ck
{
    ...
    __global__ void
        kernel_reduce_multiblock(const InGridDesc_M_K                in_grid_desc_m_k,
    ...
    struct HipTensorGridwiseReduction_mk_to_m_multiblock
    {
        static_assert(
                                   ...
                                   OutDataType* const __restrict__ p_out_value_global)
        {
            ...
            if(thread_k_cluster_id == 0)
            {
                if(!float_equal_zero{}(beta))
                {
                    ...
                    static_for<0, MThreadSliceSize, 1>{}([&](auto I) {
                        auto currVal = type_convert<AccDataType>(priorDstValueBuf[I]);
                        prior_dest_elementwise_op(currVal, currVal);
                        accu_value_buf(I) += currVal * beta;
                    });
                };
        ...
        __device__ static void
            RunWithIndex(const InGridDesc_M_K&               in_grid_desc_m_k,
                         const OutGridDesc_M&                out_grid_desc_m,
                         const InElementwiseOperation        in_elementwise_op,
                         const PriorDestElementwiseOperation prior_dest_elementwise_op,
                         const AccElementwiseOperation       acc_elementwise_op,
                         ...
                         IndexDataType* const __restrict__ p_out_index_global)
        {
            using BlockwiseReduceWithIndex = PartitionedBlockwiseReductionWithIndex<
                AccDataType,
                IndexDataType,
                BlockSize,
            ...
            StaticBuffer<AddressSpaceEnum::Vgpr, AccDataType, MThreadSliceSize, true>
                accu_value_buf;
            StaticBuffer<AddressSpaceEnum::Vgpr, IndexDataType, MThreadSliceSize, true>
                accu_index_buf;
            ...
            if constexpr(HaveIndexInput)
            {
                ...
                do
                {
                    ...
                    threadwise_src_val_load.MoveSrcSliceWindow(in_grid_desc_m_k,
                                                               in_thread_copy_step);
                    threadwise_src_idx_load.MoveSrcSliceWindow(in_grid_desc_m_k,
                                                               in_thread_copy_step);
            ...
            if(thread_k_cluster_id == 0)
            {
                if(!float_equal_zero{}(beta))
                {
                    ...
                    static_for<0, MThreadSliceSize, 1>{}([&](auto I) {
                        prior_dest_elementwise_op(priorDstValueBuf[I], priorDstValueBuf[I]);
