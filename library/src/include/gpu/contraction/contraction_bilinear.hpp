...
// Copyright 2018-2023, Advanced Micro Devices, Inc. All rights reserved.
...
void add_device_contraction_bilinear_m2_n2_k2_xdl_c_shuffle_f32_f32_f32_f32_kknn_instance(
    std::vector<std::unique_ptr<DeviceContractionMultipleD<
        2, 2, 2, F32, F32, F32_Tuple, F32, PassThrough, PassThrough, Bilinear>>>
        &instances);
...
void add_device_contraction_bilinear_m2_n2_k2_xdl_c_shuffle_f64_f64_f64_f64_kknn_instance(
    std::vector<std::unique_ptr<DeviceContractionMultipleD<
        2, 2, 2, F64, F64, F64_Tuple, F64, PassThrough, PassThrough, Bilinear>>>
        &instances);
...
template <index_t NumDimM, index_t NumDimN, index_t NumDimK, typename ADataType,
          typename BDataType, typename DDataType, typename EDataType>
struct DeviceOperationInstanceFactory<
    ck::tensor_operation::device::DeviceContractionMultipleD<
        ...
        EDataType, ck::tensor_operation::element_wise::PassThrough,
  ...
  static auto GetInstances() {
    ...
    if constexpr (is_same_v<ADataType, double> &&
                  is_same_v<BDataType, double> &&
